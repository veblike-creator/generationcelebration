...
# ТОКЕНЫ - замени BOT_TOKEN на свой!
...
async def generate_photo(msg: types.Message, state: FSMContext):
    ...
    try:
        ...
        # ФИКС: Проверяем images
        if hasattr(message, 'images') and message.images:
            img_data = message.images[0]
            if isinstance(img_data, dict):
                img_url = img_data.get('image_url', {}).get('url', '')
            elif hasattr(img_data, 'image_url'):
                img_url = img_data.image_url.url
            else:
                img_url = str(img_data)
            
            if ',' in img_url:
                b64_img = img_url.split(",", 1)[1]
                img_bytes = base64.b64decode(b64_img)
        
        # Парсим base64 из текста
        if not img_bytes and message.content:
            img_match = re.search(r'image/[a-z]+;base64,([A-Za-z0-9+/=]+)', str(message.content))
            if img_match:
                img_bytes = base64.b64decode(img_match.group(1))
        
        if img_bytes:
            photo_file = BufferedInputFile(img_bytes, "result.png")
            await msg.answer_photo(photo_file, caption=f"✅ Осталось: {remaining-1}")
            use_limit(user_id)
        else:
            await msg.answer(f"❌ Не удалось. Ответ: {str(message.content)[:100]}")
...
async def text_generate(msg: types.Message):
    ...
    try:
        ...
        if img_bytes:
            ...
            await msg.answer(f"❌ Ошибка. Ответ: {str(message.content)[:100]}")
